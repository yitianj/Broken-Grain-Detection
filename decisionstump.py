...
class DecisionTreeNode:
    ...
    def __init__(self, data_idx, the_depth, attr_list, split_attr, max_depth = 5):
        self.left = None
        self.right = None
        self.data = data_idx
    ...
    def train(self):
        ...
        if len(best_right_idx) == 0 or len(best_left_idx) == 0:
            ...
            self.left = DecisionTreeNode(best_left_idx, self.depth+1, self.attr_list, self._attr)
            self.left.train()
            self.right = DecisionTreeNode(best_right_idx, self.depth+1, self.attr_list, self._attr)
            self.right.train()
        return 
...
if __name__ == "__main__":
    ...
    for run in range(100):
        ...
        DT = DecisionTreeNode(train_, 0, attr_list, split_attr = 7)
        DT.train()
...
print(np.mean(avg_train_acc), np.std(avg_train_acc))
print(np.mean(avg_test_acc), np.std(avg_test_acc))
print(np.mean(avg_test_f1), np.std(avg_test_f1))
print(np.mean(avg_test_rec), np.std(avg_test_rec))
print(np.mean(avg_test_prec), np.std(avg_test_prec))
print(DT.split_attr)
#print()
#embed()
